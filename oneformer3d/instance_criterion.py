...
class InstanceCriterion:
    ...
    def __call__(self, pred, insts):
        ...
        """
        cls_preds = pred['cls_preds']  #[400,4]
        pred_scores = pred['scores']   #[400,1]
        pred_masks = pred['masks']  #[400, voxel size]
        ...
        for i in range(len(insts)):
            ...
            if insts[i].get('query_masks') is not None:
                gt_instances.query_masks = insts[i].query_masks
            indices.append(self.matcher(pred_instances, gt_instances)) #HungarianMatcher
...
class InstanceCriterionForAI:
    ...
    def __init__(self, matcher, loss_weight, fix_dice_loss_weight, iter_matcher, fix_mean_loss=False):
        self.matcher = TASK_UTILS.build(matcher)
        #class_weight = [1] * num_classes + [non_object_weight]
        #self.class_weight = class_weight
        self.loss_weight = loss_weight
    ...
    def get_layer_loss(self, aux_outputs, insts, indices=None):
        ...
        for mask, score, inst, (idx_q, idx_gt) in zip(pred_masks, pred_scores,
            ...
            pred_mask = mask[idx_q]
            tgt_mask = inst.sp_masks[idx_gt]
            ratio_mask = inst.ratio_inspoint[idx_gt]
            mask_bce_losses.append(F.binary_cross_entropy_with_logits(
            pred_mask, tgt_mask.float()))
            mask_dice_losses.append(dice_loss(pred_mask, tgt_mask.float()))
            ...
            pred_score = score[idx_q]
            with torch.no_grad():
                tgt_score = get_iou_with_crop(pred_mask, tgt_mask, ratio_mask).unsqueeze(1)
    ...
    def __call__(self, pred, insts):
        ...
        for mask, score, inst, (idx_q, idx_gt) in zip(pred_masks, pred_scores,
            ...
            #ratio_mask = torch.zeros(idx_gt.size(), device=idx_gt.device)
            #for i, idx in enumerate(idx_gt):
            #    ratio_mask[i] = inst.ratio_inspoint[int(idx)]
...
@MODELS.register_module()
class InstanceCriterionForAI_OneToManyMatch:
    """Instance criterion.
    ...
    def get_layer_loss(self, aux_outputs, insts, indices=None):
        ...
        for mask, score, inst, (idx_q, idx_gt) in zip(pred_masks, pred_scores,
            ...
            # No need to distinguish between positive and negative samples, compute for all masks
            ...
            # Calculate score losses
            if score is not None:
                pred_score = score[idx_q]

                tgt_score = torch.zeros_like(pred_score)  # Default score for negative samples is 0
                if (idx_gt != -1).any():
...
class SparseMatcher:
    ...
    def __call__(self, pred_instances, gt_instances, **kwargs):
        ...
        cost_value = torch.where(
            gt_instances.query_masks, cost_value, self.inf)
...
class One2ManyMatcher:
    """Match queries to their corresponding ground truth objects using query_masks.

    Args:
        costs (List[Callable]): Cost functions (no longer used).
        topk (int): Limit topk matches per query (no longer used).
    """
    ...
    @torch.no_grad()
    def __call__(self, pred_instances, gt_instances, **kwargs):
        """Match pre masks to GT based on query_masks.
        ...
        Args:
            ...
            gt_instances (:obj:`InstanceData_`): Ground truth which can contain
                `labels` of shape (n_gts,), `masks` of shape (n_gts, n_points),
                `query_masks` of shape (n_queries, n_gts).

        Returns:
            Tuple:
                Tensor: Query ids of shape (n_queries,), 
                Tensor: GT ids of shape (n_queries,) with -1 for unmatched queries.
        """
        query_masks = gt_instances.query_masks
        n_queries, n_gts = query_masks.shape

        # If there are no GTs, return all queries as unmatched (-1)
        if n_gts == 0:
            return torch.arange(n_queries), torch.full((n_queries,), -1)
        ...
        # For each query, check if it's matched with any GT in query_masks
        matched_queries = torch.any(query_masks, dim=1).to(query_masks.device)
