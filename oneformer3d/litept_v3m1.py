...
"""
LitePT V3m1 implementation for ForestFormer Fine-tuning.
Contains the user-provided PointTransformerV3 code adapted for ForestFormer.
"""
...
litept_path = "/cluster/scratch/atopaloglu/forestpoint/LitePT"
...
# Import PointROPE from LitePT libs
try:
    from libs.pointrope.pointrope import PointROPE
except ImportError:
    print("Warning: Could not import PointROPE from libs.pointrope.pointrope. Trying libs.pointrope...")
    try:
        from libs.pointrope import PointROPE
    except ImportError:
         print("Error: Could not import PointROPE. Make sure LitePT libs are available.")
         PointROPE = None
...
class SerializedAttention(PointModule):
    def __init__(
        ...
        # rope
        if PointROPE is not None:
            self.rope = PointROPE(freq=rope_freq)
        else:
            self.rope = None
            print("Warning: PointROPE not initialized because it could not be imported.")
    ...
    def forward(self, point):
        ...
        if not self.enable_flash:
            ...
            if self.rope is not None:
                q = self.rope(q.float(), pos).to(qkv.dtype) # hack here workround to make rope cuda float32 happy
                k = self.rope(k.float(), pos).to(qkv.dtype) # hack here workround to make rope cuda float32 happy
            ...
            # 1. reshape to [batch_size x nheads x ntokens x head_dim]
            ...
            # rope
            if self.rope is not None:
                q = self.rope(q.float(), pos).to(q.dtype) # hack here workround to make rope cuda float32 happy
                k = self.rope(k.float(), pos).to(k.dtype) # hack here workround to make rope cuda float32 happy
            ...
            input_tensor = torch.cat([
                q.reshape(-1, H, C // H).unsqueeze(1),
                k.reshape(-1, H, C // H).unsqueeze(1),
                v.reshape(-1, H, C // H).unsqueeze(1),
            ], dim=1)
...
@MODELS.register_module("PT-v3m1-gridpool-xcpe-xattn-rope-pretrain")
class PointTransformerV3(PointModule):
    def __init__(
        ...
        enc_cpe=(True, True, True, True, True),
        enc_attn=(True, True, True, True, True),
        enc_rope_freq=(100.0, 100.0, 100.0, 100.0, 100.0),
        dec_depths=(2, 2, 2, 2),
        dec_channels=(64, 64, 128, 256),
        dec_num_head=(4, 4, 8, 16),
        dec_patch_size=(48, 48, 48, 48),
        dec_cpe=(True, True, True, True),
        dec_attn=(True, True, True, True),
        ...
        grid_size=0.05, # Default, can be overridden by config
        pretrained=None, # For loading weights
    ):
        super().__init__()
        ...
        self.enc_cpe = enc_cpe
        self.enc_attn = enc_attn
        self.dec_cpe = dec_cpe
    ...
    def load_pretrained(self, pretrained_path):
        """
        Load pretrained weights. Uses param.copy_() like ptv3_backbone to handle
        spconv weight format differences ([out,in,k,k,k] vs [out,k,k,k,in]) by
        permuting before copy, avoiding load_state_dict shape mismatch errors.
        """
        ...
        # Strip prefixes to get backbone keys (enc.xxx, dec.xxx, embedding.xxx)
        prefixes_to_strip = ['module.student.backbone.', 'module.backbone.', 'backbone.', 'module.']
        ckpt_state = {}
        for k, v in state_dict.items():
            ...
            for prefix in prefixes_to_strip:
                if new_key.startswith(prefix):
                    ...
                    break
            ckpt_state[new_key] = v

        # Permutations for 5D conv: [out,in,k,k,k] <-> [out,k,k,k,in]
        permutations = [(0, 2, 3, 4, 1), (0, 4, 1, 2, 3)]
        loaded, permuted, skipped = 0, 0, 0
        ...
        with torch.no_grad():
            for name, param in self.named_parameters():
                ...
                if ckpt.shape == param.shape:
                    param.copy_(ckpt)
                    loaded += 1
                elif len(ckpt.shape) == 5 and len(param.shape) == 5:
                    matched = False
                    for perm in permutations:
                        p = ckpt.permute(*perm).contiguous()
                        if p.shape == param.shape:
                            param.copy_(p)
                            loaded += 1
                            permuted += 1
                            print(f"[LitePT] Permuted {name}: {ckpt.shape} -> {p.shape}")
                            matched = True
                            break
                    if not matched:
                        print(f"[LitePT] SKIP {name}: ckpt {ckpt.shape} vs model {param.shape}")
                        skipped += 1
                else:
                    print(f"[LitePT] SKIP {name}: shape mismatch ckpt {ckpt.shape} vs model {param.shape}")
                    skipped += 1
            ...
            for name, buf in self.named_buffers():
                if name not in ckpt_state:
                    continue
                ckpt = ckpt_state[name]
                if ckpt.shape == buf.shape:
                    buf.copy_(ckpt)
                    loaded += 1

        print(f"[LitePT] Loaded {loaded} weights, permuted {permuted}, skipped {skipped}")

    def forward(self, input):
        # Handle input from ForestFormer (SparseConvTensor)
        if isinstance(input, spconv.SparseConvTensor):
             # Extract data from sparse tensor
            features = input.features  # (N, in_channels)
            indices = input.indices    # (N, 4) - [batch_idx, x, y, z]
            ...
            data_dict = {
                ...
                "coord": coords * self.grid_size, # Reconstruct approx original coords
                "batch": batch_indices,
                "offset": offset,
            ...
            # Assume input is already a dict or Point
            point = Point(input)
        ...
        # Handle output for ForestFormer (List of Tensors)
        if isinstance(input, spconv.SparseConvTensor):
