    ...
    def compute_metrics(self, results):
        ...
        for eval_ann, single_pred_results in results:
            ...
            if 'originids' in single_pred_results:
                eval_ann['pts_semantic_mask'] = eval_ann[
                    'pts_semantic_mask'][single_pred_results['originids']]
                eval_ann['pts_instance_mask'] = eval_ann[
                    'pts_instance_mask'][single_pred_results['originids']]
            ...
            if self.metric_meta['dataset_name'] == 'S3DIS':
                ...
                for thing_cls in self.thing_class_inds:
                    pan_gt['pts_semantic_mask'][
                        pan_gt['pts_semantic_mask'] == thing_cls] = \
                        thing_min_value
        ...
        max_value = max(
            max(self.thing_class_inds), max(self.stuff_class_inds)) + 1
        if not ignore_index:
            ret_sem = seg_eval(
                gt_semantic_masks_sem_task,
                pred_semantic_masks_sem_task,
        ...
        if self.metric_meta['dataset_name'] == 'S3DIS':
            ...
            # ForAINetV2 instance evaluation is binary tree-vs-nontree.
            # Current dataset meta has no trailing unlabeled class, so keep
            # only the first thing class id instead of slicing away the tail.
            thing_valid_class_ids = tuple(
                self.valid_class_ids[idx] for idx in self.thing_class_inds[:1])
            ret_inst = instance_seg_eval(
                gt_semantic_masks_inst_task,
                gt_instance_masks_inst_task,
                pred_instance_masks_inst_task,
